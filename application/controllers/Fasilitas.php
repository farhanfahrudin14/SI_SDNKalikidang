...
class Fasilitas extends CI_Controller {
	...
	public function ajax_list()
	{
		...
		foreach($list as $li){
			...
			if($li->photo){
				$row[] = '<a href="'.base_url('img/fasilitas/'.$li->photo).'" target="_blank">
							<img src="'.base_url('img/fasilitas/'.$li->photo).'" class="img-responsive" style="max-height:150px; max-width:400px;"/>
						  </a>';
			}else{
				$row[] = '(No photo)';
			}
	...
	public function add()
	{
		...
		// Tambahkan validasi opsional untuk deskripsi
		...
		if($this->form_validation->run() == false){
			...
			$data = [
				...
				'description' => $this->input->post('description', true), // ← tambahan deskripsi
			];
...
{
    $data['fasilitas'] = $this->db->get_where('fasilitas', ['id' => $id])->row();

    if (!$data['fasilitas']) {
        show_404();
    }

    $this->load->view('fasilitas/detail', $data);
}
