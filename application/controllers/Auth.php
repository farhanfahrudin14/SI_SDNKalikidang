	...
	public function login()
{
    ...
    $this->form_validation->set_rules('role', 'Role', 'required'); // role dropdown
    ...
    if ($this->form_validation->run() === TRUE)
    {
        ...
        if ($this->ion_auth->login($identity, $password, $remember))
        {
            ...
            // Mapping dropdown role -> nama grup yang diperbolehkan
            $role_allowed_groups = [
                'admin' => ['admin', 'admin_biasa'], // dropdown admin cocok untuk kedua grup
                'Guru'  => ['Guru'],                 // dropdown Guru hanya cocok untuk grup Guru
            ];

            $allowed = $role_allowed_groups[$role] ?? [];
            $has_access = false;

            foreach ($groups as $group) {
                if (in_array($group->name, $allowed)) {
                    $has_access = true;
                    break;
                }
            }
            ...
            if (!$has_access) {
                ...
                $this->session->set_flashdata('message', 'Anda tidak memiliki akses ke role yang dipilih.');
                redirect('auth/login', 'refresh');
            }

            // Simpan user ke session, termasuk profil
            $this->session->set_userdata($user);
            $this->session->set_userdata('role', $role); // simpan role dropdown juga
...
public function reset_password_manual($id)
{
    // Cek user
    ...
    $this->form_validation->set_rules('new', 'Password Baru', 'required|min_length[6]');
    $this->form_validation->set_rules('new_confirm', 'Konfirmasi Password', 'required|matches[new]');

    if ($this->form_validation->run() === FALSE) {
        $this->data['user'] = $user;
        $this->_render_page('auth/reset_password_manual', $this->data);
    } else {
        ...
        if ($this->ion_auth->reset_password($user->email, $new_password)) {
            ...
            redirect('auth/reset_password_manual/'.$id);
        }
    }
}
	...
	public function create_user()
{
    ...
    $this->form_validation->set_rules('role', 'Role', 'required');  // ROLE WAJIB DIPILIH
    ...
    if ($this->form_validation->run() === TRUE)
    {
        ...
        // Ambil role dari dropdown
        $selected_role = $this->input->post('role'); // 3 atau 4

        // REGISTER USER + MASUKKAN KE GROUP SESUAI ROLE
        if ($this->ion_auth->register($identity, $password, $email, $additional_data, [$selected_role]))
        {
            $this->session->set_flashdata('message', $this->ion_auth->messages());
	...
	public function edit_user($id)
{
    ...
    // Jika bukan admin, user hanya boleh edit dirinya sendiri
        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            if ($this->ion_auth->is_admin()) {
                ...
                if ($this->input->post('groups')) {
                    foreach ($this->input->post('groups') as $grp) {
